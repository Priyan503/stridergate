...
function Overview({ stats }) {
  ...
  useEffect(() => { 
    fetch('/api/ml/predictive-forecast').then(r => r.json()).then(d => { if (d.success) setForecast(d.data); }); 
  }, []);
      ...
      <div className="grid2" style={{ marginBottom: 20 }}>
        ...
        <div className="card">
          <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span>✨ AI Predictive Analytics Insight</span>
          </div>
          {forecast ? (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 13, lineHeight: 1.5, marginBottom: 14 }}>{forecast.aiInsight}</div>
              <div className="grid2" style={{ gap: 10 }}>
                <div style={{ background: 'var(--surface2)', padding: '10px', borderRadius: 8, textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Projected Claims</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)' }}>{forecast.projectedClaims}</div>
                </div>
                <div style={{ background: 'var(--surface2)', padding: '10px', borderRadius: 8, textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Expected Loss Ratio</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--safe)' }}>{forecast.forecastLossRatio}</div>
                </div>
              </div>
            </div>
          ) : (
             <div style={{ marginTop: 12, color: 'var(--muted)', fontSize: 13 }}><span className="spinner" style={{width: 12, height:12, borderWidth: 2, marginRight: 6}} />Analyzing historical data...</div>
          )}
        </div>
...
function Workers({ showToast }) {
  ...
  const [newWorker, setNewWorker] = useState({ name: '', city: '', zone: '', platform: 'Swiggy', earnings: '', plan: 'Standard' });
  ...
  const handleAdd = async (e) => {
    ...
    if (data.success) {
      ...
      setNewWorker({ name: '', city: '', zone: '', platform: 'Swiggy', earnings: '', plan: 'Standard' });
      showToast(`${data.data.name} added successfully ✓`, 'success');
    } else {
      showToast(data.error || 'Failed to add rider', 'error');
    }
  };
  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
        <button className="btn btn-sm" style={{ background: 'var(--accent)', color: '#000', fontWeight: 600, padding: '6px 14px', borderRadius: 8, border: 'none', cursor: 'pointer' }} onClick={() => setShowAddForm(!showAddForm)}>
          {showAddForm ? 'Cancel' : '+ Add Rider'}
        </button>
      </div>

      {showAddForm && (
        <form className="card fade-in" onSubmit={handleAdd} style={{ marginBottom: 20, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end', border: '1px solid var(--accent)' }}>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Name</label>
            <input required placeholder="Rider Name" value={newWorker.name} onChange={e => setNewWorker({...newWorker, name: e.target.value})} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', outline: 'none' }} />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>City</label>
            <input required placeholder="City" value={newWorker.city} onChange={e => setNewWorker({...newWorker, city: e.target.value})} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', width: 120, outline: 'none' }} />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Zone</label>
            <input required placeholder="Zone" value={newWorker.zone} onChange={e => setNewWorker({...newWorker, zone: e.target.value})} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', width: 120, outline: 'none' }} />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Platform</label>
            <select value={newWorker.platform} onChange={e => setNewWorker({...newWorker, platform: e.target.value})} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', outline: 'none' }}>
          ...
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Earnings P/W</label>
            <input required type="number" placeholder="Amount" value={newWorker.earnings} onChange={e => setNewWorker({...newWorker, earnings: e.target.value})} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', width: 90, outline: 'none' }} />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>Plan</label>
            <select value={newWorker.plan} onChange={e => setNewWorker({...newWorker, plan: e.target.value})} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', outline: 'none' }}>
          ...
          <div>
            <button type="submit" className="btn btn-sm" style={{ background: 'var(--text)', color: 'var(--surface)', padding: '7px 16px', borderRadius: 6, border: 'none', fontWeight: 600, cursor: 'pointer' }}>Save Rider</button>
          </div>
        </form>
      )}
...
function Fraud({ showToast }) {
  ...
  const analyze = (w) => {
    ...
    fetch(`/api/ml/analyze-fraud`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workerData: w }) })
      .then(res => res.json())
      .then(data => {
         if (data.success) {
           setResult(data.data);
        ...
        <div>
          {analyzing && (
            <div className="card fade-in" style={{ textAlign: 'center', padding: 40 }}>
              ...
              <div style={{ fontSize: 14, color: 'var(--muted)' }}>✨ Running AI-Powered Telemetry Analysis...</div>
...
export default function AdminDashboard({ activePage, showToast }) {
  ...
  const titles = {
    overview: { title: 'Insurer Dashboard', sub: 'Shielded Rider Operations · Week of Apr 1–7, 2026' },
