...
export default function LoginPage({ onLogin }) {
  ...
  useEffect(() => {
    fetch('/api/auth/workers')
      ...
      .catch(() => setWorkers([
        { workerId: 'W001', name: 'Arun Kumar',   platform: 'Swiggy' },
        { workerId: 'W002', name: 'Shruthi Mehra', platform: 'Zomato' },
        { workerId: 'W003', name: 'Kmunar Sinha',  platform: 'Swiggy' },
        { workerId: 'W004', name: 'Nithin Kumar',   platform: 'Zomato' },
      ]));
  }, []);
  ...
  const doLogin = async (role) => {
    ...
    try {
      ...
      const user = role === 'worker'
        ? workers.find(w => w.workerId === selWorker) || { name: 'Demo Worker', workerId: 'W001' }
        : { name: 'Shielded Rider Admin' };
      onLogin(role, user);
    }
    setLoading(null);
  };
    ...
    <div className="login-page">
      ...
      <div className="login-box">
        ...
        <div style={{ textAlign: 'center', marginTop: 24, fontSize: 12, color: 'var(--muted)' }}>
          Shielded Rider v2.0 · All data is demo/mock · No real money transferred
        </div>
      </div>
    </div>
  );
