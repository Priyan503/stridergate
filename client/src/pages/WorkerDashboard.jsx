...
export default function WorkerDashboard({ worker: initialWorker, activePage, showToast }) {
  ...
  if (activePage === 'dashboard') return (
    <div className="page fade-in">
      ...
      {/* Intelligent AI Dashboard Banner */}
      <div className="card" style={{ marginBottom: 20, background: 'linear-gradient(135deg, rgba(0,212,168,0.1), rgba(0,0,0,0))', border: '1px solid rgba(0,212,168,0.3)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 24 }}>✨</span>
          <div>
            <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--safe)' }}>Intelligent Dashboard: Earnings Protected</div>
            <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 4 }}>Your weekly coverage is ACTIVE. AI monitoring confirms you are actively protected against current weather disruptions. Guaranteed instant mock-payout via UPI.</div>
          </div>
        </div>
      </div>
                    ...
                    <td>
                      {c.status === 'paid' ? 
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          ...
                          <span style={{ fontSize: 10, color: 'var(--safe)', border: '1px solid var(--safe)', borderRadius: 10, padding: '2px 6px', display: 'inline-block', textAlign: 'center' }}>⚡ Instant UPI Payout</span>
                        </div> : 
                        <StatusBadge status={c.status} />
                      }
                    </td>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10 }}>
                ...
                {c.status === 'paid' && (
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: 12, color: 'var(--safe)', fontWeight: 600 }}>✓ Auto-Approved by AI</span>
                    <span style={{ fontSize: 11, color: 'var(--muted)' }}>⚡ Instant payout routed to Mock UPI</span>
                  </div>
                )}
                {c.status === 'rejected' && (
